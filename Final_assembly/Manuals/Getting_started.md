# Getting Started with TinyZ280
This is a quick start guide to get TinyZ280 power up and running.

## Setup Hardware
![annotated](../tinyz280_annotated.jpg)
Refer to picture above for location of various hardware features.

- TinyZ280 needs a 5V power supply with maximum current of 500mA. A TTL-level serial adapter with Transmit and Receive and ground signals. No handshake signals are required. The terminal program parameters are: 57600 baud, Odd parity, 8 data bit, 1 stop and no handshake.
- To boot from the CF disk, be sure the mode jumer is inserted.
- The included CF already has all the necessary software installed in track 0. CPM2.2 distribution files are on drive B, CPM3 distribution files are on drive A & C.
- Refer to manual for software installation on a new CF
- When power is applied, the expected current is 300mA-400mA
- The RTC (U9) is not populated but the board is ready for the DS12887 RTC. The 128 registers of DS12887 are mapped to Z280's I/O space 0x00xx00-0x00xx7F
...
>
```
Detailed description of ZZMon commands are in [ZZMon manual](ZZMon_V07.md).
...
## RAM Drive
Drive E is the RAM drive. The 3.5meg CP/M RAM drive is physically located in memory 0x80000-0x3FFFFF. RAM drive's directory needs to be initialized to 0xE5. This is done with ZZMon using the command:
