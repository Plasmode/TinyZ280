# ZZMon (TinyZ280 Monitor) V0.7 Manual
March 2018

## Introduction

ZZMon is the monitor program for TinyZ280. In normal mode of operation it is stored in track 0 sector 0xF8-0xFF of the compact flash card. When power is initially applied, the cold bootstrap code which is stored in the bootsector of the compact flash card will copy the ZZMon from its location in CF to memory (0x400-0xFFF) and transfer control to ZZMon. When a new CF disk is first installed in the TinyZ280, remove the boot mode jumper will enable the UART Bootstrap where ZZMon can be loaded serially at 57600 baud, odd parity, no handshakes. When loaded serially this way, ZZMon can initialize the new CF disk with cold bootstrap code in the bootsector and copy ZZMon into track 0, sector 0xF8-0xFF. With boot mode jumper installed, the initialized CF can operate in the normal mode.

## ZZMon commands

ZZMon is a simple monitor with the following single-key commands. Except when noted, the commands may be entered in upper or lower cases. In the following description, command entered is in **bold**.
...
**Z**
```
zero memory to 0xFFFE
press Return to execute command
```
Fill memory from 0x1000 to 0xFFFE with 0x0. Press carriage return to confirm the command execution; press other key to abort the command
...
press Return to execute command
```
Fill memory from 0x1000 to 0xFFFE with 0xFF. Press carriage return to confirm the command execution; press other key to abort the command

**T**
```
test RAM up to 0xFFFE
press Return to execute command
```
...
3–CP/M3:
```
Enter '2' to boot CP/M 2.2 or enter '3' to boot CP/M3. This assumes the appropriate software has been copied to track 0 of CF as described under the “C” command. Confirm the command with a carriage return or abort command with any other key stroke.
```
boot CP/M
2–CP/M2.2,
...
3–CP/M3:
```
Prior to the execution of the C2 command, CP/M2.2 BDOS/CCP/BIOS must be loaded in memory 0xDC00-0xFFFF.

Prior to the execution of the C3 command, CP/M3 CPMLDR must be loaded in memory starting from 0x1100.

Location of programs on CF track 0 (sector values are LBA):

CFMonLdr: boot sector (sector 0). CFMon is loaded to memory 0x1000
