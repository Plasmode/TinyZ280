...
New CPLD with CF bootstrap state machine (CFinit) and modified memory map. Here is the CPLD [schematic](tinyzramcf.pdf) in PDF format and the [programming binary](tinyzramcf_CPLD_binary.zip) file. The state machine design is rather convoluted. Here is the theory of operation.
